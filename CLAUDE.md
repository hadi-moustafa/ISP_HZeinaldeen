...
- `Address`/`Building` fold into the single `subscribers.address` line (comma-joined); `Region` is matched/auto-created against the `regions` reference table into `subscribers.region_id` (see the address/nationality overhaul below — this replaced an older `subscriber_addresses` table that no longer exists).
- `Password`/`Switch`/`Mac Address`/`Nationality` land in `subscribers.import_metadata jsonb` — a judgment call to avoid a schema change for low-value fields (the import file's free-text "Nationality" doesn't reliably map onto `subscribers.nationality`'s two-value Lebanese/Syrian selector, so it's kept as metadata, not written to that column).
...
- Commit is a single Postgres RPC (`import_subscribers_batch`, `supabase/migrations/0004_subscriber_import.sql` + `0005_import_address_line2.sql`, updated by `0011_address_region_nationality.sql`), matching the `postpone_invoice()` pattern — one transaction, so a mid-import failure never leaves partial data.
...
- **Explicit column-mapping step (2026-07-31)**: uploading a file now goes upload → **columns** → preview → result, not straight to preview. `readWorkbook()`/`defaultColumnMapping()`/`applyColumnMapping()` in `src/lib/api/import.ts` read the file's actual header row (via `sheet_to_json(sheet, { header: 1 })`, not `Object.keys()` of the first data row, so it works even with zero data rows) and best-guess match each one to a canonical DB field by wording alone — case/whitespace/order-insensitive, matching client instruction that column order varies between exports but the wording itself doesn't. Anything that doesn't match (a genuinely different header phrase) starts unmapped and is surfaced to the admin on the **columns** step as a plain dropdown per detected header ("Don't import this column" / one of the 19 canonical fields), so a header a specific export phrases differently is never silently read as blank — the admin fixes it once, per file. `Continue` is blocked until Username and Name both have a column and no two columns are mapped to the same field. Verified live end-to-end (login, real file upload, an intentionally misworded header staying unmapped until fixed by hand, a correctly-worded-but-reordered header auto-matching, and a full import committing to the real Supabase project) with all test data cleaned up afterward.
- **Full column set, one DB field per Excel column (2026-07-31)**: client gave the definitive, authoritative list of 20 export columns and asked for a real `subscribers` column per column — no more jsonb metadata catch-all. `supabase/migrations/0012_full_import_columns.sql` adds `subscribers.password`, `switch`, `mac_address`, `building`, `price`, `balance`, `company_id`, backfills `password`/`switch`/`mac_address` from the old `import_metadata` jsonb, and drops that column (nothing left to hold once everything has a real field).
  - **Reseller vs Company split**: these are two distinct columns now, not one. Reseller → `owners` (who the subscriber's account belongs to, via `subscribers.owner_id`) — low-stakes, auto-created by name inside `import_subscribers_batch` if it doesn't exist yet, same treatment as `regions`, never blocks an import. Company → `companies` (the network operator, e.g. Nova) via the new `subscribers.company_id` — high-stakes like Service, must resolve to an existing company or be explicitly mapped by the admin in the preview step (the "Unrecognized Company values" section, formerly keyed off Reseller). Company also disambiguates which `services` row to use when a service name exists under more than one company — the role Reseller used to play before this split.
  - `company_id` is a **denormalized mirror** of the assigned service's own company (`services.comp_id`), not an independently free-floating field — kept in sync at write time (`SubscriberFormPage`'s Company select auto-narrows the Service dropdown to that company and clears an invalid selection; the import RPC resolves both from the same Company column). Deliberate choice to avoid two sources of truth silently diverging.
  - **Price/Balance are per-subscriber override fields** (`subscribers.price`/`balance`), explicit client decision — independent of `services.sell_price`/`paid_price` (the shared default for everyone on that plan). They are informational only and do **not** feed invoice generation; `createInvoice()`/the monthly cron still price off the assigned service, unchanged.
  - **Nationality normalization**: `normalizeNationality()` in `src/lib/api/import.ts` maps common variants (`"LB"`, `"lebanese "`, `"Syria"`, etc., case/whitespace-insensitive prefix match) onto the two values `subscribers.nationality`'s `CHECK` constraint accepts (`'Lebanese'`/`'Syrian'`); anything else is left blank rather than blocking the import — nationality isn't required for the app to function.
  - A blank Company column on a row is a new `missing_company` `RowIssue`, treated the same as `missing_username` (row excluded from the importable set, shown in the invalid-rows count, not silently dropped) — added because Company is now load-bearing (used to disambiguate Service and to populate `company_id`), unlike before when a blank Reseller just meant an unmatched company that could be admin-mapped per file rather than per row.
  - Verified live end-to-end via the real app (login, a file using every one of the 20 canonical headers, owner auto-creation, region auto-creation, company/service resolution, and all six new columns landing correctly) and separately via the subscriber create form (Company→Service filtering, all new fields round-tripping to the detail page). All test data cleaned up afterward.
...
## Pay button: Mark Paid / Postponed / Debt + Arabic WhatsApp notifications

The subscriber list's Pay button (`SubscribersListPage.tsx`) opens a 3-mode "Update status" modal instead of a single payment form — explicit client walkthrough, each mode does its DB action then opens `wa.me` with an Arabic message via `src/lib/whatsapp.ts` (shared with `InvoicesSection.tsx`'s existing receipt-share button):

- **Paid**: logs a full payment (existing `createPayment`, unchanged) → the `sync_invoice_status` trigger (see below) turns the card green and auto-renews expiry → WhatsApp opens with an Arabic message confirming payment, with today's date/time in Arabic (`toLocaleDateString('ar-LB', ...)`).
- **Postponed**: calls the existing `postpone_invoice` RPC with a picked date → card turns orange → WhatsApp opens with an Arabic message stating the new due date.
- **Debt**: new `doubleNextMonthInvoice()` in `lib/api/invoices.ts` — upserts *next* month's invoice at `2 × services.sell_price` (creates it early if the monthly cron hasn't yet, or raises the amount if it has). Never touches the current invoice, which stays unpaid/red on its own. Reverts to the normal price automatically the month after, since nothing here changes `services.sell_price` — only that one invoice row was doubled. WhatsApp opens with an Arabic message naming the doubled amount.

New subscribers default `default_collector_id` to the collector named "hussien" (client-specified default) in `SubscriberFormPage.tsx` — looked up by name from the loaded collectors list, only applied on create, never overwrites an existing subscriber's collector on edit.

**Real-number testing note**: this feature was verified against 4 real subscribers seeded with real WhatsApp numbers the client provided (kept in the DB for the client's own manual click-through, not deleted like other seed data) — but verification itself never actually sent a message to a real phone. `window.open` was stubbed in the browser during automated testing to capture the constructed `wa.me` URL/message instead of letting it navigate, so the DB effects (invoice status, expiry date, doubled next-month amount) and the exact Arabic message text were both confirmed correct without contacting a real person from an unattended script. Actually sending is still just the collector tapping Send in WhatsApp themselves, same zero-infrastructure model as the original receipt-share flow.
...
## Address, nationality, regions, bulk actions, always-available Pay (2026-07-31)

- **Subscriber addressing was collapsed to one line.** The old `subscriber_addresses` table (multiple labeled addresses per subscriber, each with line1/line2/city/region/country, with its own CRUD modal on the detail page) is **dropped**. Subscribers now carry `address TEXT` and `region_id UUID REFERENCES regions(id)` directly. Explicit client instruction: "the address is only one line no need for details," and region should be "a dropdown that we can choose from" rather than freely typed. `supabase/migrations/0011_address_region_nationality.sql` backfilled `address`/`region_id` from each subscriber's primary (or first) old address row before dropping the table.
- **`regions` is a new admin-managed reference table** (`src/pages/admin/RegionsPage.tsx`, `src/lib/api/regions.ts`), same CRUD pattern as collectors/owners (`name`, `is_active`). Subscriber form/list filter both pick from it via a dropdown.
- **`national_id` was renamed to `nationality`** and repurposed as a `CHECK`-constrained selector with exactly two values, `'Lebanese'` or `'Syrian'` — explicit client instruction ("the national id has 2 options lebanese, syrian"), confirmed as *replacing* the free-text ID field entirely, not adding a nationality field alongside a kept ID number. Any pre-existing value that wasn't literally one of those two strings (e.g. real ID numbers previously typed into the free-text field) was dropped to `NULL` by the migration — accepted data loss, since the old field's data doesn't map onto the new one's meaning.
- **Excel import updated to match**: `Address`/`Building` now fold into the single `subscribers.address` line (`Building` has no column of its own anymore, so it's appended rather than dropped), and `Region` is matched/auto-created against the new `regions` table inside `import_subscribers_batch` (an unresolved region isn't worth blocking an import over, unlike Reseller/Service). This is a minimal fix to keep the importer working against the new schema — the deeper import robustness work the client separately asked to hold off on ("let's finish these first then we can tackle the excel") is still pending.
- **Subscriber list multiselect gained Delete and Deactivate bulk actions** (`bulkDeleteSubscribers`/`bulkSetConnectionStatus` in `lib/api/subscribers.ts`, plain PostgREST `.in('id', ids)` calls, no RPC needed). Deactivate sets `connection_status = 'suspended'` (not `'cancelled'`) — reversible, matching "deactivate" rather than the permanent removal that Delete already covers.
- **The Pay button on the subscriber list is now always clickable**, even when the subscriber has no invoice yet for the current period (e.g. created mid-month). `openPaymentModal()` in `SubscribersListPage.tsx` creates the current period's invoice on the fly (same `createInvoice()` used at subscriber-creation time) before opening the modal, when the subscriber is active and has a service. If there's no service to bill against, the button still opens the modal for an ad-hoc payment with `invoice_id: null`, same as the underlying `payments` table already allows.
